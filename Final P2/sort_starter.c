...
// number of entries is getting close to MAX defined NUM
		if (ctotal == (NUM - 1)) {
			void **hold1;

			hold1 = realloc(records, sizeof *records * (ctotal * 8));
			if (hold1 == NULL) {
				printf(
						"Realloc failed trying to add more space for file data. Exiting\n");
				fflush(stdout);
				pthread_exit((void *) -1);
			} else {
				printf("reallocing TMP!\n");
	...
	// number of entries to be added in the final struct is greater than capaticty
	pthread_mutex_lock(st->lock);
			int sz = st->final[0].line_number;
			printf("sz = %d\n",sz);
			//printf("Address @ 1 %p\n", st->final);
			if (sz > 11111) {
				void **hold2;
				...
				add_more = sz + (ctotal * 8);
				printf("2 add_more = %d\n", add_more);
				}
				//printf("the Movie title is %s.\n", st->final[1].mTitle);
				//pthread_exit((void*)-1);
				hold2 = realloc(st->final, sizeof(st->final) * add_more);

				printf("Address  of hold2 %p\n", hold2);
				if (hold2 == NULL) {
					printf(
							"Realloc failed trying to add more space for main struct. Exiting\n");
					fflush(stdout);
					pthread_exit((void *) -1);
				} else {
					printf("reallocing MS!\n");
					st->final = (struct mData *) hold2;
					printf("Address @ 2 %p\n", st->final);
					//printf("Address @ 3 %p\n", st->final);
					//printf("Address of wDir @ 1 %p\n",st->wDir);
					//printf("The wDir name is %s\n", st->wDir);
					//printf("The oDir name is %s\n", st->oDir);
					//printf("the Movie title is %s.\n", records[ctotal].mTitle);
